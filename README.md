...
| `WASD` / 矢印 | 移動 |
| `Shift` | 走る |
| `Space` / `Enter` | 足元のランドマークを調べる／街に入る |
| `B` | 港で乗船・下船（海に出られる） |
| `Esc` | 街を出る（表示中は閉じる） |
...
ワールドマップで東京に立ち、`Space` を押すと街へ入ります。`Esc` で元の場所へ戻ります。
戻り先は入る前に立っていたマスで、ワールドマップの状態はそのまま保たれます。
...
どの地点から街へ入れるかは `assets/landmarks.json` の `map` で決まります。
別の街を作った場合は、対応するランドマークに `"map": "assets/町のファイル.bin"`
を足せば入れるようになります。
